...
import { Youtube, Twitch, RefreshCw, Trophy, Swords, Calendar } from 'lucide-react';
...
const MatchesSection: React.FC = () => {
  ...
  if (matches.length === 0) {
    return (
      <section id="matches" className="min-h-screen bg-transparent px-4 md:px-8 py-24 relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative">
          ...
          <div className="text-center py-12 bg-purple-900/20 backdrop-blur-sm rounded-xl">
            ...
            <p className="text-gray-400 max-w-md mx-auto px-4">
              There are no live or upcoming matches at the moment. Check back later for exciting tournaments and matchups!
            </p>
          </div>
        </div>
      </section>
              ...
              <div className="relative p-4 md:p-6">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
                    ...
                    <div className="relative">
                      ...
                      {match.team1.logo_url ? (
                        ...
                        <div className="relative w-16 h-16 rounded-full bg-purple-900/50 flex items-center justify-center border-2 border-purple-500/30">
                          <span className="text-xl font-bold text-white">{match.team1.name[0]}</span>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    <span className="text-4xl font-bold text-white">{match.team1.score}</span>
                    <div className="w-12 h-12 rounded-full bg-purple-900/50 flex items-center justify-center">
                      <Swords className="w-6 h-6 text-purple-400" />
                    </div>
                    <span className="text-4xl font-bold text-white">{match.team2.score}</span>
                  </div>

                  {/* Team 2 */}
                  <div className="flex flex-col md:flex-row items-center gap-4 flex-1 justify-end text-center md:text-right">
                    <span className="text-white font-bold text-xl order-2 md:order-1">{match.team2.name}</span>
                    <div className="relative order-1 md:order-2">
                      ...
                      {match.team2.logo_url ? (
                        ...
                        <div className="relative w-16 h-16 rounded-full bg-purple-900/50 flex items-center justify-center border-2 border-purple-500/30">
                          <span className="text-xl font-bold text-white">{match.team2.name[0]}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    {match.status === 'live' && (
                      <a
                        ...
                        className="px-4 py-1.5 rounded-full text-white text-sm font-medium bg-purple-500/20 
